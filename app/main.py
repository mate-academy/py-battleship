...
class Ship:
    ...
    def _create_decks(self, start: tuple[int], end: tuple[int]) -> None:
        column_coord, row_coord = (start[0], end[0]), (start[1], end[1])
        rows_range = range(min(column_coord), max(column_coord) + 1)
        columns_range = range(min(row_coord), max(row_coord) + 1)
